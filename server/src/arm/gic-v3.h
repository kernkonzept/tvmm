...
public:
  ...
  void prio(unsigned char p, Prio_mapper *m, Vcpu_handler *vcpu)
  {
    _irq.prio(p);
  ...
  void group(bool grp1, Vcpu_handler *vcpu)
  {
    _irq.group(grp1);
    rebind(vcpu);
  }
...
private:
   void rebind(Vcpu_handler *vcpu) const
   {
     ...
     h->bind(vcpu->thread_cap(), cfg.raw);
   }
...
private:
  ...
  void irq_mmio_write(Irq &irq, unsigned rgroup, l4_uint32_t value)
  {
    switch (rgroup)
      {
      case R_group:    irq.group(value, &_cpu);               return;
      ...
      case R_prio:     irq.prio(value & _prio_mask, _prio_mapper, &_cpu); return;
